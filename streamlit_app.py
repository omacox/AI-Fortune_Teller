...
def process_transcript(transcript):
    ...
    st.session_state['responses'].append(
        f"{st.session_state['current_question']}:" + transcript)
    # st.write(st.session_state['text'])
    if st.session_state['current_question'] < 7:
        st.session_state['current_question'] += 1
        image_display.image(
            f'questions/{st.session_state["current_question"]}.png')
    else:
        st.session_state['run'] = False
    ...
    # )
        ...
        webbrowser.open_new_tab(
            f'https://docs.google.com/forms/d/e/1FAIpQLSeTOg8m-Wd4rnG5WKVPnQ6dbuEM8oCDSbAvbNVsD0KWWv7z0A/viewform?usp=pp_url&entry.1826277827={ai_response}')
                ...
                try:
                    ...
                    if json.loads(result_str)['message_type'] == 'FinalTranscript':
                        ...
                        transcription_display.markdown(
                            f'<p style="font-size: 20px;">Real-time Transcript: {result}</p>', unsafe_allow_html=True)
                        # st.write(st.session_state['text'])
